  ...
  return (
    <div>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-line bg-paper p-4"><div className="text-xs text-ink-500">Bills issued</div><div className="text-lg font-bold">{invoices.length}</div></div>
        <div className="rounded-2xl border border-line bg-paper p-4"><div className="text-xs text-ink-500">Total billed</div><div className="text-lg font-bold">{formatGHS(totalBilled)}</div></div>
        <div className="rounded-2xl border border-line bg-paper p-4"><div className="text-xs text-ink-500">Total collected</div><div className="text-lg font-bold text-brand-800">{formatGHS(totalCollected)}</div></div>
      </div>
        ...
        {pendingTellers.map((p) => (
          <div key={p.id} className="flex items-center justify-between rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm">
            ...
            <div className="flex gap-2">
              <form action={confirmTellerPayment}><input type="hidden" name="paymentId" value={p.id} /><button type="submit" className="rounded-full bg-forest px-3 py-1.5 text-xs font-medium text-white hover:bg-forest-deep">Confirm</button></form>
