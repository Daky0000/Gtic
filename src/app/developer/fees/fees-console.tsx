...
import { formatGHS, formatUSDEquivalent, parseProcessingFeePercent, parseUsdRate } from "@/lib/money";
import { getIntFee, getSetting, SETTING_KEYS } from "@/lib/settings";
import {
  addFeeItem, createFeeSchedule, deleteFeeItem, saveCurrencyRate, saveProcessingFee,
...
/**
 * Shared fees & pricing UI, rendered from both /developer/fees (developer —
 * every section) and /admin/fees (system admin — everything except the
 * currency multiplier and processing fee, which stay developer-only
 * decisions). `basePath` is threaded into every form as a hidden `returnTo`
 * so a save lands back on whichever page rendered it, and `isDev` gates the
 * two developer-only sections.
 */
export async function FeesConsole({
  basePath,
  isDev,
  searchParams,
}: {
  ...
  const { error, saved } = searchParams;

  const [cycles, hostels, schedules, years, docTranscript, docAttestation, docVerification, libraryFine, rateRaw, processingFeeRaw, shortCourses] =
    await Promise.all([
      ...
      getSetting(SETTING_KEYS.PROCESSING_FEE_PERCENT),
      db.shortCourse.findMany({ orderBy: { name: "asc" } }),
    ]);
  const rate = parseUsdRate(rateRaw);
  const processingFeePercent = parseProcessingFeePercent(processingFeeRaw) ?? 1;

  return (
    <div className="mx-auto max-w-3xl">
      <h1 className="text-2xl font-bold">Fees &amp; pricing</h1>
      <p className="mt-1 text-sm text-ink-500">
        Charged in GHS. Changes apply to bills generated after the change.
        {!isDev && " The currency multiplier and checkout processing fee are managed by the developer account."}
      </p>
      ...
      {isDev && (
        <>
          ...
          {/* Processing fee */}
          <section className="mt-6 rounded-2xl border border-line bg-paper p-5">
            <h2 className="font-semibold text-brand-800">Checkout processing fee</h2>
            <p className="mt-1 text-xs text-ink-500">
              A percentage added on top of a fee only at the moment someone actually pays online — the
              sticker price shown while applying/browsing never changes. e.g. a GHS 50 fee becomes GHS 50.50
              to charge at 1%. Defaults to 1% until changed.
            </p>
            <form action={saveProcessingFee} className="mt-3 flex flex-wrap items-end gap-3">
              <input type="hidden" name="returnTo" value={basePath} />
              <label className="text-xs text-ink-600">
                Fee percentage
                <input
                  name="percent" type="number" step="0.01" min="0" max="100"
                  defaultValue={processingFeePercent} placeholder="1"
            ...
            <p className="mt-2 text-xs text-ink-500">
              Currently GHS 50.00 becomes <strong>{formatGHS(Math.round(5000 * (1 + processingFeePercent / 100)))}</strong> at checkout.
            </p>
          </section>
        </>
      )}
